...
public class DrawingTree {
	...
	private final int ROOTBORDER = 40;	
	private final int DOWNMARGIN = 40;	
	
	private double graphicNodeSize = 0;
	...
	public DrawingTree(Pane paneTree, DoubleProperty animationSpeed, ReadOnlyDoubleProperty stageWidthProperty, ReadOnlyDoubleProperty stageHeightProperty,  WindowController windowController) {
	 ...
	 * @param root
	 */
	public void insertRoot(INode root){
		newIGraphicNode = root.getGraphicNode();
		graphicNodeSize = newIGraphicNode.getRadiusSize();
	...
	public void insertNode(Result result, int value) {
		...
		if (result.getSide() == Side.LEFT) {
			xAnimatedNode.bind(newIGraphicNode.getParent().getX().subtract(graphicNodeSize));	
		} else {
			xAnimatedNode.bind(newIGraphicNode.getParent().getX().add(graphicNodeSize));	
		}
	...
	private void createBranch(IGraphicNode node) {
		...
		Line line = new Line();
		line.startXProperty().bind(node.getParent().getX().add(graphicNodeSize / 2));
		line.startYProperty().bind(node.getParent().getY().add(stackPaneHeight / 2));
		line.setStroke(Color.WHITE);
		
		if (node.getSide() == Side.LEFT) {
			line.endXProperty().bind(node.getParent().getX().subtract(graphicNodeSize / 2));	
		} else {
			line.endXProperty().bind(node.getParent().getX().add(graphicNodeSize * 1.5));	
		}
	...
	private void balanceTreeNext() {
		...
		if (iGraphicNode.getX().get() != xAnimatedNode.get()) {		
			if (iGraphicNode.getSide() == Side.LEFT) { // vypočítám nový x pro větev
				xAnimatedBranch.bind(iGraphicNode.getParent().getX().subtract(graphicNodeSize / 2).subtract(graphicNodeSize * iGraphicNode.getRightChildrenCount()));
			} else {
				xAnimatedBranch.bind(iGraphicNode.getParent().getX().add(graphicNodeSize * 1.5).add(graphicNodeSize * iGraphicNode.getLeftChildrenCount()));
			}			
	...
	private void redraw() {	
		...
		xAnimatedBranch.bind(root.getX().add(graphicNodeSize / 2));
		...
		if (nullNode != null && nullNode.getValue().equals("NULL")) { //na závěr přebinduju nullnode
			...
			if (nullNode.getSide() == Side.LEFT) {
				xAnimatedNode.bind(nullNode.getParent().getX().subtract(graphicNodeSize));	
				xAnimatedBranch.bind(nullNode.getParent().getX().subtract(graphicNodeSize / 2));	
			} else {
				xAnimatedNode.bind(nullNode.getParent().getX().add(graphicNodeSize));	
				xAnimatedBranch.bind(nullNode.getParent().getX().add(graphicNodeSize * 1.5));	
			}
	...
	private void redBlackDeleteInfo() {
		switch ((int)recordOfAnimations.get(animationIndex).getObject()) {
		case 1:
	...
	private void highlightNodeAnimationFinished() {
		if(!notFind && ((boolean)recordOfAnimations.get(animationIndex).getObject())) {
			if (animationSpeed.get() == 0) { //když nebude animace
				text.setText("HLEDÁNÍ PRVKU: " + value + "\n • Prvek byl nalezen!");
			} else {
				text.setText("HLEDÁNÍ PRVKU: " + value +"\n • Prvek byl nalezen!" + "\n" + oldText);
	...
	private void deleteNodeAnimation() {
		...
		if ((boolean) recordOfAnimations.get(animationIndex).getObject()) { //pokud má děti
	...
	private void moveNodeAnimation() {
		IGraphicNode graphicNodeRemoved = recordOfAnimations.get(animationIndex).getNode1(); //zaloha už je 
		IGraphicNode graphicNodeMoved = (IGraphicNode) recordOfAnimations.get(animationIndex).getObject();
	...
	private void moveValueAnimation() {
		IGraphicNode graphicNodeRemoved = recordOfAnimations.get(animationIndex).getNode1();
		IGraphicNode graphicNodeMoved = ((IGraphicNode) recordOfAnimations.get(animationIndex).getObject());
	...
	private void updateFactor() {
		...
		if (animationSpeed.get() == 0 || (!(boolean)recordOfAnimations.get(animationIndex).getObject())) { //když nebude animace
		...
		AVLGraphicNode start = (AVLGraphicNode)recordOfAnimations.get(animationIndex).getNode1();
	...
	private void reColor() {
		...
		RedBlackGraphicNode node = (RedBlackGraphicNode) recordOfAnimations.get(animationIndex).getNode1();
		trees.Color color = (trees.Color) recordOfAnimations.get(animationIndex).getObject();		
	...
	/*	if (!(isBalance) && animationSpeed.get() != 0) {
		...
		if (((Side)recordOfAnimations.get(animationIndex).getObject()) == Side.LEFT) {
			xAnimatedNode.bind(recordOfAnimations.get(animationIndex).getNode1().getX().subtract(graphicNodeSize));
			recordOfAnimations.get(animationIndex).getNode1().setLeft(nullNode);
		} else {
			xAnimatedNode.bind(recordOfAnimations.get(animationIndex).getNode1().getX().add(graphicNodeSize));	
			recordOfAnimations.get(animationIndex).getNode1().setRight(nullNode);
		}
		
		yAnimatedNode.bind(recordOfAnimations.get(animationIndex).getNode1().getY().add(DOWNMARGIN));
		...
		nullNode.setParent(recordOfAnimations.get(animationIndex).getNode1());
	...
	private void setDoubleBlack() {
		...
		if ((recordOfAnimations.get(animationIndex).getNode1() != null) && recordOfAnimations.get(animationIndex).getObject() == null) {
			...
			} else if (recordOfAnimations.get(animationIndex).getObject() != null) {				
				appendNewText("\n • Kořen je vždy černý.");				
				nullNode.removeDoubleBlackHighlight();
	...
	private void rrAnimation() {		
		IGraphicNode nodeB = recordOfAnimations.get(animationIndex).getNode1();
		...
		xAnimatedNode = new SimpleDoubleProperty(nodeB.getX().get() + graphicNodeSize);	
	...
	private void rrAnimationFinished(IGraphicNode nodeA, IGraphicNode nodeB) {	
		if (!isRedBlack) {
			((AVLNode)recordOfAnimations.get(animationIndex).getObject()).computeFactor();
		}		
		...
		if (nodeA.getParent() == null) {
			...
			if (nodeA.getSide() == Side.LEFT) {
				xAnimatedNode.bind(nodeA.getParent().getX().subtract(graphicNodeSize));
			} else if(nodeA.getSide() == Side.RIGHT) {			
				xAnimatedNode.bind(nodeA.getParent().getX().add(graphicNodeSize));
			}		
	...
	private void rlAnimation() {
		IGraphicNode nodeC = recordOfAnimations.get(animationIndex).getNode1();
		...
		xAnimatedNode = new SimpleDoubleProperty(nodeC.getX().get() + graphicNodeSize);	
	...
	private void rlAnimationFinished(IGraphicNode nodeA, IGraphicNode nodeB, IGraphicNode nodeC) {
		...
		if (nodeB.getParent() == null) {
			...
			if (nodeB.getSide() == Side.LEFT) {
				xAnimatedNode.bind(nodeB.getParent().getX().subtract(graphicNodeSize));
			} else if(nodeB.getSide() == Side.RIGHT) {			
				xAnimatedNode.bind(nodeB.getParent().getX().add(graphicNodeSize));
			}			
